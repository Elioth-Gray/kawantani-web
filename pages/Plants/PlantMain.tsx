...
const PlantMain = () => {
  ...
  const filteredPlants = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      filtered = filtered.filter(plant =>
        plant.nama_tanaman.toLowerCase().includes(searchQuery.toLowerCase()) ||
        plant.deskripsi_tanaman?.toLowerCase().includes(searchQuery.toLowerCase())
      );
    }
          ...
          {selectedFilters.length > 0 && (
            <div className="flex flex-wrap gap-2">
              ...
              {selectedFilters.map((filterId) => {
                const category = categories.find(cat => cat.id_kategori_tanaman === filterId);
                return (
                  <span
                    key={filterId}
            ...
            {filteredPlants.length > 0 ? (
              filteredPlants.map((plant) => (
                <PlantCard
                  ...
                  description={plant.deskripsi_tanaman || "Tidak ada deskripsi tersedia"}
                  onClickHandler={() => navigateToPlantDetail(plant.id_tanaman)}
                  difficulty={plant.tingkat_kesulitan}
                  duration={plant.durasi_penanaman}
                  sunlight={plant.kebutuhan_sinar_matahari}
                  water={plant.kebutuhan_air}
                />
              ))
            ) : (
              <div className="col-span-3 text-center py-12">
                <div className="text-gray-500">
                  ...
                  <h3 className="text-xl font-semibold mb-2">Tidak ada tanaman ditemukan</h3>
